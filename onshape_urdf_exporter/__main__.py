...
def main():
    ...
    if config['outputFormat'] == 'urdf':
        ...
        print(Fore.RED + 'ERROR: Unknown output format: ' +
              config[
                  'outputFormat'] + ' (supported are urdf and sdf)' + Style.RESET_ALL)
        exit()
    ...
    def addPart(occurrence, matrix):
        ...
        if part['partId'] == '':
            print(Fore.YELLOW + 'WARNING: Part ' + part[
                'name'] + ' has no partId' + Style.RESET_ALL)
            return
        ...
        if config['color'] is not None:
            ...
            metadata = client.part_get_metadata(
                part['documentId'], part['documentMicroversion'], part['elementId'],
            ...
            for entry in metadata['properties']:
                if 'value' in entry and type(entry['value']) is dict and 'color' in \
                        entry['value']:
            ...
            if prefix in config['dynamicsOverride']:
                ...
                if part['partId'] not in massProperties['bodies']:
                    print(Fore.YELLOW + 'WARNING: part ' +
                          part[
                              'name'] + ' has no dynamics (maybe it is a surface)' + Style.RESET_ALL)
                    return
                ...
                if abs(mass) < 1e-9:
                    print(Fore.YELLOW + 'WARNING: part ' +
                          part[
                              'name'] + ' has no mass, maybe you should assign a material to it ?' + Style.RESET_ALL)
    ...
    def buildRobot(tree, matrix):
        ...
        print(Fore.BLUE + Style.BRIGHT +
              '* Adding top-level instance [' + instance[
        ...
        for occurrence in occurrences.values():
            if occurrence['assignation'] == tree['id'] and occurrence['instance'][
                'type'] == 'Part':
    ...
    with open(config['outputDirectory'] + '/robot.' + robot.ext, 'w',
              encoding="utf-8") as stream:
